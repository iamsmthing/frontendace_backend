...
export const getChallengeById = async (req: Request, res: Response) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ error: "ID is required as a query parameter." });
    }
...
export const fetchUserScores = async (req: Request, res: Response) => {
  try {
    let leaderboard_score = await db.query.users.findMany({
      columns: {
        score: true,
        id: true,
        username: true,
        ...
        progress:{
            ...
            with: {
                challenge: {
                  columns: {
                    id: true,
                    difficulty: true, // Fetching difficulty
                  },
                },
              },
        }
    ...
    return res.status(200).json(leaderboard_score);
  } catch (error) {
    return res.status(500).json({ error: "Failed to fetch leaderboard score" });
  }
};
